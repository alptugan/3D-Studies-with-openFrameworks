...
{
    float depth = LinearizeDepth( texture(range, pos).r ) * 20000.;
...
void main()
{
    ...
    float rangeTexel = FocalValue( vTexCoord );
...
//    int samples = (fast == 1) ? 11 : 21;
	...
	for( int s = 0; s < samples; ++s ) {
        ...
        vec4 texel = texture( tex, vTexCoord + baseOffset + offset ) * (1. - abs( FocalValue(vTexCoord + baseOffset + offset)  - rangeTexel) );
        sum += texel.rgb * weights[s];
		offset += blurOffset;
	}
    
	vFragColor.rgb = sum * globalColor.rgb;
    vFragColor.a = globalColor.a;
}